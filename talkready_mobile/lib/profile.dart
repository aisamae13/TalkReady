    ...
    try {
      ...
      if (pickedFile != null) {
        ...
        if (user != null) {
          ...
          // ✅ NEW: Clean the base64 string (remove data URI prefix if present)
          final cleanBase64 = profilePicBase64.contains(',')
              ? profilePicBase64.split(',').last
              : profilePicBase64;

          await _firestore.collection('users').doc(user.uid).update({
            'profilePicBase64': cleanBase64, // ✅ Store clean base64
            'profilePicSkipped': false,
          });

          setState(() {
            _profilePicBase64 = cleanBase64; // ✅ Use clean base64 in state
