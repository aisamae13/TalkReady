...
class _AIBotScreenState extends State<AIBotScreen> {
  ...
  final List<Map<String, dynamic>> _nonSimulationMessages = []; // For non-simulation mode
  final List<Map<String, dynamic>> _simulationMessages = []; // For simulation mode
  List<Map<String, dynamic>> get _messages => _isInSimulation ? _simulationMessages : _nonSimulationMessages;
  ...
  static const String _ttsServerUrl = 'https://0d37-103-149-37-6.ngrok-free.app/tts';
    ...
    _fetchOnboardingData().then((_) {
     ...
     WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        ...
        logger.i('Adding greeting to _messages: {"text": "$randomGreeting", "isUser": false}');
        setState(() {
  ...
  String _generateRandomGreeting() {
    ...
    final List<String> baseGreetings = [
      if (_accentLocale == 'en_US') "$timePrefix, $_userName! How’s your day been? Spill something fun!",
      if (_accentLocale == 'en_GB') "$timePrefix, $_userName! How’s your day going? Tell me a smashing story!",
      if (_accentLocale == 'en_AU') "$timePrefix, $_userName! How’s your day? Got any ripper tales?",
    ];
  ...
  Future<void> _initializeTts() async {
    ...
    await _flutterTts.setLanguage(_accentLocale ?? 'en_US'); // Use fetched locale or fallback
  ...
  Future<void> _requestPermissions() async {
    if (!await Permission.microphone.isGranted) {
      ...
      if (status != PermissionStatus.granted && mounted) {
        _showSnackBar('Microphone permission denied. Please enable it in settings.');
      }
    }
  }
  ...
  try {
    ...
    if (userData != null && userData.containsKey('onboarding')) {
      ...
      if (dailyGoal != null) {
        String minutesStr = dailyGoal.replaceAll(RegExp(r'[^0-9]'), ''); // Extract only numbers
        int minutes = int.tryParse(minutesStr) ?? 5; // Default to 5 minutes if parsing fails
        _timeGoalSeconds = minutes * 60;
        logger.i('Set timeGoalSeconds to: $_timeGoalSeconds seconds ($minutes minutes)');
      } else {
        logger.i('No dailyPracticeGoal found, using default timeGoalSeconds: $_timeGoalSeconds');
      }
      ...
      if (mounted) {
        setState(() {
          ...
          if (_userName == null || _userName!.isEmpty) {
            ...
            logger.w('userName not found in onboarding, using fallback: $_userName');
          }
          ...
          _remainingSeconds = (userData['remainingSeconds'] as int?) ?? _timeGoalSeconds;
          logger.i('Loaded progress: sessionProgress=$_sessionProgress, responseCount=$_responseCount, remainingSeconds=$_remainingSeconds');

         // Load messages from Firestore, separating by mode
          _nonSimulationMessages.clear();
          _simulationMessages.clear();
          (userData['nonSimulationMessages'] as List<dynamic>?)?.forEach((msg) {
      ...
      if (mounted) _showSnackBar('No onboarding data found. Please complete onboarding.');
    }
  } catch (e) {
  ...
  _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
    ...
    if (_remainingSeconds > 0) {
      setState(() => _remainingSeconds--); // Safe setState with mounted check above
    } else {
      timer.cancel();
      _stopListening();
      if (mounted) {
        _showSnackBar('Time’s up for today’s practice!'); // This SnackBar appears
...
void _showContinuePracticeDialog() {
  ...
  showDialog(
    ...
    builder: (context) => AlertDialog(
      ...
      content: const Text('You’ve reached today’s practice goal. Would you like to continue practicing?'),
      actions: [
        TextButton(
          onPressed: () {
            Navigator.pop(context); // Close dialog
            _showSnackBar('Practice session ended for today.'); // This SnackBar appears
...
void _resetTimerAndContinue() {
  if (mounted) {
    setState(() {
      _remainingSeconds = _timeGoalSeconds; // Reset to the daily goal or a custom value
      _hasStartedListening = false; // Reset to allow new timer start
    });
    _startTimer(); // Restart the timer
    _showSnackBar('You can continue practicing for another ${_formatTime(_timeGoalSeconds)}!');
  }
}
  Future<void> _startListening() async {
    if (!_isListening && !_isTyping && _remainingSeconds > 0) {
      ...
      logger.d('Starting to listen. IsListening: $_isListening, IsTyping: $_isTyping, RemainingSeconds: $_remainingSeconds');
  ...
  if (_textController.text.isNotEmpty && _remainingSeconds > 0) {
    ...
    setState(() {
      _messages.add({'text': processedText, 'isUser': true, 'isSimulation': _isInSimulation});
      _pruneMessages();
    });
    ...
    if (_audioFilePath != null) {
      try {
        ...
        if (!await file.exists() || await file.length() == 0) {
          ...
          logger.w('Audio file check failed: exists=${await file.exists()}, length=${await file.length()}');
          return;
        }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (secureUrl == null || secureUrl.isEmpty) {
          _showSnackBar('Cloudinary upload returned an invalid URL. Please check settings.');
          throw Exception('Invalid Cloudinary response');
        }
        return secureUrl;
      }
      _showSnackBar('Cloudinary upload failed: Status ${response.statusCode}');
      throw Exception('Failed to upload to Cloudinary: Status ${response.statusCode}');
    } catch (e) {
 ...
 Future<String> _getOpenAIResponse(String prompt, {bool isSimulation = false, String? userInput}) async {
    if (!dotenv.isInitialized) {
      logger.e('DotEnv not initialized');
      _showSnackBar('Environment variables not loaded. Please check app configuration.');
    ...
    try {
      logger.d('Sending request to OpenAI with prompt: $prompt, userInput: $userInput, isSimulation: $isSimulation');
      ...
      logger.d('OpenAI response status: ${response.statusCode}, body: ${response.body}');
      if (response.statusCode == 200) {
        final responseBody = utf8.decode(response.body.runes.toList(), allowMalformed: true);
        final data = jsonDecode(responseBody);
        final aiResponse = data['choices'][0]['message']['content'] ?? 'Interesting! How can I help you today?';
        final cleanedAiResponse = _cleanText(aiResponse);
        logger.i('OpenAI response: $cleanedAiResponse');
        return cleanedAiResponse;
      } else {
        logger.w('OpenAI failed with status: ${response.statusCode}, body: ${response.body}');
        _showSnackBar('OpenAI API request failed: Status ${response.statusCode}');
...
String _processText(String text) {
 ...
 Future<void> _generateAIResponse(String userInput) async {
  ...
  try {
    ...
    double pronunciationScore = 0.0; // Still placeholder (requires speech analysis)
    double interactionScore = await _analyzeInteraction(userInput, _messages);

    if (mounted) {
      setState(() {
        _sessionProgress['Fluency'] = (_sessionProgress['Fluency']! + fluencyScore) / _responseCount;
        _sessionProgress['Grammar'] = (_sessionProgress['Grammar']! + grammarScore) / _responseCount;
        _sessionProgress['Pronunciation'] = (_sessionProgress['Pronunciation']! + pronunciationScore) / _responseCount;
        _sessionProgress['Vocabulary'] = (_sessionProgress['Vocabulary']! + vocabScore) / _responseCount;
        _sessionProgress['Interaction'] = (_sessionProgress['Interaction']! + interactionScore) / _responseCount;
      });
...
Future<double> _analyzeFluency(String text) async {
  try {
    ...
    return double.tryParse(response.trim()) ?? 0.1; // Default to 0.1 if parsing fails
  } catch (e) {
...
Future<double> _analyzeInteraction(String text, List<Map<String, dynamic>> history) async {
  try {
    final historyString = jsonEncode(history.map((msg) => {'text': msg['text'], 'isUser': msg['isUser']}).toList());
    final response = await _getOpenAIResponse(
  ...
  Future<void> _speakText(String text, {required bool isUser}) async {
    ...
    setState(() => _isProcessingTTS = true); // Start showing processing indicator
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _showSnackBar('F5-TTS server error (status ${response.statusCode}), using default TTS.');
      ...
      setState(() => _isProcessingTTS = false); // Stop showing processing indicator
    }
  }
  ...
  Future<void> _flutterTtsFallback(String text) async {
    try {
      ...
      await _flutterTts.setLanguage(_accentLocale ?? 'en_US'); // Match user’s accent or default to US
...
void _saveProgress({bool isStartingSimulation = false, bool showSnackBar = true}) {
  if (mounted) {
    final user = FirebaseAuth.instance.currentUser;
    if (user != null) {
      FirebaseFirestore.instance.collection('users').doc(user.uid).update({
        ...
        'messages': _isInSimulation ? _simulationMessages : _nonSimulationMessages,
      }).then((value) {
        ...
        if (!isStartingSimulation) {
          Navigator.pop(context, _sessionProgress); // This navigates back to homepage
        }
      }).catchError((error) {
...
void _endSimulation() {
  logger.i('Before ending simulation, _simulationMessages: $_simulationMessages');
  setState(() {
    ...
    if (_nonSimulationMessages.isEmpty && _initialGreeting != null) {
      _nonSimulationMessages.insert(0, {'text': _initialGreeting!, 'isUser': false});
    }
  ...
  logger.i('After ending simulation, _simulationMessages: $_simulationMessages, _nonSimulationMessages: $_nonSimulationMessages');
  // Iwasan ang pag-speak ng greeting ulit kung hindi kailangan
  if (_nonSimulationMessages.length == 1 && _initialGreeting != null) {
    _speakText(_initialGreeting!, isUser: false); // Speak lamang kung ito lang ang mensahe
  }
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                 ...
                 Divider(color: Colors.blue.shade200, thickness: 1.0, height: 1.0),
                  _buildSimulationOption(
                    context: context,
...
Widget _buildSimulationOption({
  ...
  return ListTile(
    ...
    onTap: onTap,
    shape: RoundedRectangleBorder(
      borderRadius: BorderRadius.circular(8.0), // Slight rounding for each item
    ),
...
void _startSimulation(String scenario) {
  ...
  logger.i('Starting simulation. _isInSimulation: $_isInSimulation, scenario: $scenario, remainingSeconds: $_remainingSeconds');
  setState(() {
    _isInSimulation = true;
    _messages.clear(); // Clear all messages to start fresh for simulation
    _sessionProgress.forEach((key, value) => _sessionProgress[key] = 0.0); // Reset progress
    _responseCount = 0;
  });
  ...
  Future<void> _simulateCallCenterScenario(String scenario) async {
    ...
    switch (scenario) {
      ...
      case 'advanced_upsell':
        aiPrompt = "I’m interested in your basic plan. Do you have anything better?";
        break;
...
Future<void> _evaluateSimulationResponse(String userResponse) async {
  ...
  String lastPrompt = _messages.lastWhere((msg) => !msg['isUser'], orElse: () => {'text': ''})['text'] as String;
  String scenarioType = _messages.any((msg) => msg['isSimulation'] && !msg['isUser'])
      ? _messages.firstWhere((msg) => msg['isSimulation'] && !msg['isUser'])['text']
      : 'general';
...
''';
  ...
  try {
      ...
      setState(() {
        _messages.add({'text': aiResponse, 'isUser': false, 'isSimulation': true});
      });
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isUser
              ? Colors.blue[100]
              : isSimulation
                  ? Colors.amber[100] // Amber background for simulation AI messages
                  : Colors.white,
...
class IconRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        _buildIcon(Icons.keyboard, Colors.purple.shade200, onKeyboardTap, isActive: isTyping),
        ...
        const SizedBox(width: 20),
        _buildIcon(Icons.play_circle, Colors.amber.shade200, onModeTap, isActive: false), // Use play_circle for Mode
      ],
    );
  }

  Widget _buildIcon(IconData icon, Color color, VoidCallback? onTap, {bool isActive = false}) {
    return GestureDetector(
      ...
      child: Container(
        ...
        height: 70,
        decoration: BoxDecoration(
          color: onTap != null && isActive ? color.withValues(alpha: 0.7) : color,
