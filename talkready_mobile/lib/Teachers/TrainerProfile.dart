...
// Import settings pages if they are indeed used by trainers, similar to profile.dart
// These are assumed to be the correct paths based on the existing _buildOpenContainerSettingsItem calls
...
class TrainerProfile extends StatefulWidget { // Renamed from ProfilePage
const TrainerProfile({super.key}); // Renamed from ProfilePage

@override
State<TrainerProfile> createState() => _TrainerProfileState(); // Renamed from _ProfilePageState
}

class _TrainerProfileState extends State<TrainerProfile> { // Renamed from _ProfilePageState
...
if (user != null) {
  ...
  _userStreamSubscription = _firestore
      .collection('users') // Assuming trainers are in the 'users' collection
      .doc(user.uid)
      .snapshots()
      .listen(
    (snapshot) {
      ...
      return Dialog(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: Colors.white, // <--- Remove gradient
            borderRadius: BorderRadius.circular(20),
            // No boxShadow
          ),
          child: Column(
...
try {
  ...
  if (mounted) {
    Navigator.pushNamedAndRemoveUntil(
      context,
      '/', // Navigate to login/initial route
      (route) => false,
    );
  }
} catch (e) {
...
return Scaffold(
  backgroundColor: Colors.white,
  appBar: AppBar(
    title: const Text('Trainer Profile', style: TextStyle(color: Colors.white)), // Changed title
    backgroundColor: const Color(0xFF2973B2),
    iconTheme: const IconThemeData(color: Colors.white),
    elevation: 0,
    automaticallyImplyLeading: true, // Changed to true to show back button
  ),
  body: _isLoading
            ...
            children: [
              ...
              Container(
                decoration: const BoxDecoration(
                  color: Color(0xFF2973B2), // Just a solid color, no gradient
                        ...
                        children: [
                          Container(
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(color: const Color(0xFF9CA8C7), width: 2.0), // lighter border
                              // No boxShadow
                            ),
                            child: CircleAvatar(
                              radius: 70,
...
Widget _buildSettingsTile({
  ...
  return Padding(
    padding: const EdgeInsets.only(bottom: 10.0),
    child: InkWell(
      borderRadius: BorderRadius.circular(8.0), // Changed from 10.0
      onTap: onTap,
      child: Container(
        padding: const EdgeInsets.symmetric(vertical: 14, horizontal: 16), // Changed padding
        decoration: BoxDecoration(
          color: Colors.white,
          borderRadius: BorderRadius.circular(8.0), // Changed from 10.0
          border: Border.all(color: Colors.grey.shade200), // subtle border
        ...
        child: Row(
          children: [
            Icon(icon, color: const Color(0xFF00568D), size: 22), // Changed size from 24
            const SizedBox(width: 12), // Changed width from 14
            Expanded(
              child: Text(
                title,
                style: const TextStyle(
            ...
            const Icon(Icons.arrow_forward_ios, color: Color(0xFF00568D), size: 16), // Changed size from 18
          ],
        ),
      ),
    ),
...
Widget _buildOpenContainerSettingsItem({
  ...
  const double borderRadius = 8.0; // Changed from 10.0
  return Padding(
    padding: const EdgeInsets.only(bottom: 10.0),
    child: OpenContainer(
...
return Padding(
  ...
  child: TextField(
    ...
    decoration: InputDecoration(
      ...
      filled: true,
      fillColor: const Color(0xFFF4F8FE), // Corrected Color
      border: OutlineInputBorder(
